## Use Helm and Open Service Broker API (OSBA) to deploy the po-service microservice application to AKS
...
1. Install **Service Catalog** and **Open Service Broker for Azure (OSBA)** on AKS

2. Deploy the **Azure Database for MySQL** using OSBA

3. Deploy the **po-service** microservice using Helm package manager

**Result:** A managed *MySQL* database instance on Azure and the *po-service* microservice deployed on AKS.  The microservice will use the managed MySQL instance to persist *Purchase Orders*.
